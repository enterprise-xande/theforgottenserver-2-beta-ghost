...
//player
	...
	const Position& pposition = player->getPosition();

	//old party member list
	for (Player* member : memberList) {    
		member->sendCreatureSkull(player);
//member
		Tile* mtile = member->getTile();
		const Position& mposition = member->getPosition();

		if (player->isInGhostMode() && player != member) {
			member->sendRemoveTileCreature(player, pposition, ptile->getClientIndexOfCreature(member, player));
		}
		if (member->isInGhostMode() && member != player) {
			player->sendRemoveTileCreature(member, mposition, mtile->getClientIndexOfCreature(player, member));
		}
...
//leader
	Tile* ltile = leader->getTile();
	const Position& lposition = leader->getPosition();

	leader->sendCreatureSkull(player);
	if (player->isInGhostMode() && player != leader) {
		leader->sendRemoveTileCreature(player, pposition, ptile->getClientIndexOfCreature(leader, player));
	}

	player->sendCreatureSkull(player);
	player->sendPlayerPartyIcons(leader);
	if (leader->isInGhostMode() && leader != player) {
		player->sendRemoveTileCreature(leader, lposition, ltile->getClientIndexOfCreature(player, leader));
	}
...
bool Party::joinParty(Player& player)
{
	...
	for (Player* member : memberList) {
		...
		if (member->isInGhostMode() && member != &player) {
			player.sendCreatureAppear(member, mposition, true);
		}
		if (player.isInGhostMode() && &player != member) {
			member->sendCreatureAppear(&player, pposition, true);
		}
	...
	if (player.isInGhostMode() && &player != leader) {
		leader->sendCreatureAppear(&player, pposition, true);
	}

	if (leader->isInGhostMode() && leader != &player) {
		player.sendCreatureAppear(leader, lposition, true);
	}
